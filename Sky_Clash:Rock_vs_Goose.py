...
while True:
    ...
    if playing:
        ...
        if keys[pygame.K_w] and player_rect.y > 20:
            player_rect.y -= speed
        
        # Moves the player down if it can
        if keys[pygame.K_s] and player_rect.y < 680:
            player_rect.y += speed
            
        # Moves the player left if it can
        if keys[pygame.K_a] and player_rect.x > 15:
            player_rect.x -= speed
        
        # Moves the player right if it can
        if keys[pygame.K_d] and player_rect.x < 1000:
            player_rect.x += speed

        # Moves the second player up if it can
        if keys[pygame.K_UP] > 20:
            player_2_rect.y -= 10
            
        # Moves the second player down if it can
        if keys[pygame.K_DOWN] and player_2_rect.y < 600:
            player_2_rect.y += 10
